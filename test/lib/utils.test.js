...
import { expect, LocalStorageMock } from '../test_helper';
...
describe('test the library functions', () => {
  ...
  it('retrieved the correct access token of undefined', () => {
    localStorage.setItem("hdm_state", JSON.stringify({"currentUser":{"profile":{},"accessToken":{"token_type":"bearer","expires_in":7200,"created_at":1529678369}}}))
    let g = getToken();
  ...
  it('correctly said a token has not expired yet', () => {
    ...
    var object_dict = {"currentUser":{"profile":{},"accessToken":{"access_token":"f2b43339b45eb2fb75c177ced506ccd5462d2511095e33724d8b477352b72303","token_type":"bearer","expires_in":7200,"created_at":current_time}}};
  ...
  it('could not find the created_at key and says token expired ', () => {
    localStorage.setItem("hdm_state", JSON.stringify({"currentUser":{"profile":{},"accessToken":{"access_token":"f2b43339b45eb2fb75c177ced506ccd5462d2511095e33724d8b477352b72303","token_type":"bearer","expires_in":7200}}}));
    let g = tokenExpired();
    expect(g).to.equal(true);
  });

  it('could not find the expired key and says token expired ', () => {
    localStorage.setItem("hdm_state", JSON.stringify({"currentUser":{"profile":{},"accessToken":{"access_token":"f2b43339b45eb2fb75c177ced506ccd5462d2511095e33724d8b477352b72303","token_type":"bearer"}}}));
    let g = tokenExpired();
    expect(g).to.equal(true);
  });
